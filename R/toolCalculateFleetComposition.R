...
toolCalculateFleetComposition <- function(ESdemandFVsalesLevel,
  ...
  period <- value <- share <- level <- variable <- unit <- univocalName <- subsectorL3 <- test <- NULL
  ...
  unifyDt <- merge(data.table(indexUsagePeriod = allTimeSteps)[, allYears := "all"],
                   unifyDt, by = "allYears", allow.cartesian = TRUE)[, allYears := NULL]
  ...
  # In order to obtain a starting point, it is assumed that the absolute number of new additions (equivalent to
  # the contributed energy service demand, when annual mileage stays constant as well)
  ...
  for (i in constructionYears) {
    for (j in contributionYears) {
      ...
      fleetESdemand[, eval(paste0("C", i)) := demandNewAdditions[period == startYear] * depreciationFactor,
                    by = c("region", "subsectorL3")]
  ...
  for (i in timesteps[timesteps > startYear]) {
    ...
    tmp[, eval(paste0("C", i)) := demandNewAdditions[period == i] * depreciationFactor,
        by = c("region", "subsectorL3")][, depreciationFactor := NULL]
  ...
  if (max(abs(fleetESdemand$test)) > 10^-5) {
